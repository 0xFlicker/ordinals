...
export class ElectrsBuilder extends Construct {
  constructor(scope: Construct, id: string, props: ElectrsBuilderProps) {
    ...
    const repoUrl = props.repoUrl ?? "https://github.com/ordinals/ord.git";
    ...
    const instance = new ec2.Instance(this, "BuildInstance", {
      userData,
    ...
    instance.userData.addCommands(
      ...
      `aws s3 cp ./target/release/ord s3://${bucket.bucketName}/ord-${gitRef}`,
      // Signal CloudFormation that the build has completed successfully
      `curl -X PUT --data-binary '{"Status":"SUCCESS","Reason":"Built ord","UniqueId":"OrdBuild"}' '${waitHandle.ref}'`,
    ...
    new cdk.CfnOutput(this, "ElectrsBinaryBucket", {
      value: `s3://${bucket.bucketName}/ord-${gitRef}`,
    });
  }
}
